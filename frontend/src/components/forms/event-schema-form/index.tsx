...
export function SchemaForm({readOnly = false}: Props) {
    const {activeEvent, isLoading, updateEventSchema, updateSchemaError, isUpdateSchemaPending} = useEvents();
    const router = useRouter();
    const importFileRef = useRef<HTMLInputElement>(null);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [collapsedTabs, setCollapsedTabs] = useState<Set<string>>(new Set());
    const {exportSchema, importSchema, isImporting} = useImportExport();
    ...
    const onSubmit = async (data: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            const {success} = await updateEventSchema(schemaObj);
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem
                                            ...
                                            onClick={() =>
                                                exportSchema(
                                                    Object.fromEntries(
                                                        form.getValues("tabs").map((t) => [t.name, t.columns]),
                                                    ),
                                                    "csv",
                                                )
                                            }
